...
class InfoNCELoss(nn.Module):
    ...
    def forward(self, z_i: torch.Tensor, z_j: torch.Tensor) -> torch.Tensor:
        ...
        # Never clamp the loss VALUE: torch.clamp kills the gradient once
        # saturated, freezing training at a constant loss forever. CE is
        # already >= 0 and numerically stable; guard the INPUTS instead.
        return loss
...
class VICRegLoss(nn.Module):
    ...
    def forward(self, z_a: torch.Tensor, z_b: torch.Tensor) -> torch.Tensor:
        ...
        std_loss = torch.mean(F.relu(self.gamma - std_z_a)) + torch.mean(
            F.relu(self.gamma - std_z_b)
        )
        ...
        cov_loss = (cov_z_a.pow(2).sum() - torch.diagonal(cov_z_a).pow(2).sum()) / D + (
            cov_z_b.pow(2).sum() - torch.diagonal(cov_z_b).pow(2).sum()
        ) / D
...
class CausalNextTokenLoss(nn.Module):
    ...
    def forward(
        self, ntp_logits: torch.Tensor, target_tokens: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        shift_logits = (
            text_logits[:, :-1, :].contiguous().view(-1, vocab_size)
        )  # [B*(S-1), V]
        ...
        shift_targets = torch.where(
            ...
            shift_targets,
            torch.tensor(
                self.ignore_index, device=shift_targets.device, dtype=torch.long
            ),
        )
